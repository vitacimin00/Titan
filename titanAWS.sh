...
public_ips=$(curl -s ifconfig.me || curl -s6 ifconfig.me)
...
for ip in $public_ips; do
    # Replace ":" in IPv6 with "_" for folder and container names
    ...
    do
        ...
        container_id=$(docker run -d --restart always -v "$storage_path:/root/.titanedge/storage" --name "titan_${safe_ip}_${i}" --net=host nezha123/titan-edge)

        echo -e "${GREEN}Node titan_${safe_ip}_${i} is running with container ID $container_id${NC}"
        ...
        docker exec $container_id bash -c "\
            ...
            echo 'Storage for node titan_${safe_ip}_${i} set to $storage_gb GB, Port set to $current_port'"

        docker restart $container_id

        docker exec $container_id bash -c "\
            titan-edge bind --hash=$id https://api-test1.container1.titannet.io/api/v2/device/binding"
        echo -e "${GREEN}Node titan_${safe_ip}_${i} has been bound.${NC}"
